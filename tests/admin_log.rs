...
use common::{auth_token, grant_role, insert_role, insert_user, test_context};
...
async fn migration_creates_admin_log_table() {
    ...
    let row = sqlx::query(
        "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'admin_log'",
    )
...
async fn admin_actions_are_logged_without_password_data() {
    ...
    assert_eq!(roles_updated_entry["action_info"]["target_user_id"], "user-2");
    assert_eq!(
...
async fn channel_owner_actions_are_logged() {
    ...
    assert_eq!(created_entry["action_info"]["initial_member_ids"][0], "member-1");

    let updated_entry = find_log(&logs, "channel.updated");
    assert_eq!(updated_entry["action_info"]["channel_id"], channel_id);
    assert_eq!(updated_entry["action_info"]["changes"]["name"], "ops-updated");
    ...
    assert_eq!(members_entry["action_info"]["removed_user_ids"][0], "member-1");
